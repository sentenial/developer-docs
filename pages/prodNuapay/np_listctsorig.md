---
title: List Credit Transfers (Per Originator)
keywords: List Credit Transfers (Per Originator) API
summary: "List Credit Transfers (Per Originator) RESTful API"
sidebar: ct_sidebar
permalink: np_listctsorig.html
folder: prodNuapay
toc: false
---
...
* This section deals with the call to retrieve the transactions linked to the originator.
* Click [List CTs linked to Single Beneficiary](np_listctbene.html) to view that endpoint.
...
function loadV1Docs() {
    ...
    setTimeout(function() {
        insertDocs('operation/listCreditTransfersUsingGET', '#V1-content');
        unloadRedoc();
    }, 1000);
}
...
function loadV2Docs() {
    ...
    setTimeout(function() {
        insertDocs('operation/listCreditTransfersUsingGET', '#V2-content');
        unloadRedoc();
    }, 1000);
}
